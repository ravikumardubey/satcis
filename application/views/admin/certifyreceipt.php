...
sat_case_detail as cno inner join certified_copy as cp on cp.filling_no=cno.filing_no join master_case_type as b ON cno.case_type = b.case_type_code  where filing_no='$filingNo' AND certify_number='$certify_copy_id'");
    $row22 =$case_initialization->result();
    $party_name='';
    $party_ids=$row22[0]->party_ids;
    $ids=explode(',', $party_ids);
    $filtered_array = array_filter($ids); 
    if($row22[0]->party_type==1){
        $party_name='';
        $i=1;
        foreach($filtered_array as $vals){       
            $row= $this->efiling_model->data_list_where('additional_party','party_id',$vals);
            foreach($row as  $row2){
                $party_name .=$row2->pet_name.'A-'.$i.',';              
            }
            if($vals==1){
                $party_name .=@$row->pet_name.'(A-'.$i.'),';
            }
        }
        $i++;
    }
    
    if($row22[0]->party_type==2){
        $party_name='';
        $i=1;
        foreach($filtered_array as $vals){
    ...
    $case_details=$this->db->query("select amount from aptel_certify_account_details where certify_id='{$row22[0]->certify_copy_id}'");
    ...
    <div style="position: relative;">
        ...
        <div style="overflow: hidden;">
            <div style="float: left; width: 50%;">
                <span>
                    <?php
                    $val= substr($row22[0]->filing_no,7);
                    ...
                    ?></span>
                <p> <strong>Appellant/Petitioner Name    :- </strong> <?php echo $row22[0]->pet_name.$this->efiling_model->fn_addition_party('',$filingNo,'1'); ?></p>
                <p><strong>Respondent  Name                 :-</strong><?php echo $row22[0]->res_name.$this->efiling_model->fn_addition_party('',$filingNo,'2'); ?></p>
            </div>
                <span style="float: right; width: 50%; text-align: right;">
                    <p>DATE OF APPLICATION  :<?php echo  date("d-m-Y", strtotime($row22[0]->created_on)); ?></p>
                </span>
            </div>
...
" class="table table-striped table-bordered" cellspacing="0"  width="100%" style="width:100%;border-collapse:collapse">
                        ...
                        $total=0;
                          foreach($case_details as $row1) {
                            ...
                            $i++ ; } ?>
                            <tr>
                               <td align="center"><?php echo $row22[0]->certify_number; ?></td>
                               <td align="center"><?php echo $row22[0]->year; ?></td>
