            ...
            <div class="card" style="width: 100%; padding: 0px 12px;border-top: 0px;margin-top: 0px; border-top-right-radius: 0px; border-top-left-radius: 0px;">
                ...
                <div class="row alert alert-warning" style="padding: 8px 0px 0px; font-weight: 600;font-size: 18px; margin: -20px auto 12px; max-width: 100%;">
                    ...
                    ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group required" id="div-add">
                            	...
                            	<input type="text" name="advname" value="" id=advname  class="form-control" onkeypress="serchrecordvaladv(this.value);" <?php echo $dis; ?>>
                    			<ul class="autosuggest" id="regnum_autofilldoc">
                    			</ul>
                            </div>
                    ...
                    <div class="row">
                        ...
                        <input type="button" name="" class="btn btn-success" onclick="addCouncel()" value="Add" <?php echo $dis; ?>>
                    </div>
                </fieldset>
            </div>
            </div>
...
function addCouncel(str) {
    ...
    if(advtype=='2'){
     ...
     var token_hash=HASH(token+'doccouncel');
     ...
     $.ajax({
        type: "POST",
        url: base_url+'addCouncelDoc/'+token_hash,
        data: dataa,
        cache: false,
...
$('#frmCounsel').submit(function(e){ 
    ...
    var token=Math.random().toString(36).slice(2); 
    var token_hash=HASH(token+'advsave');
	...
	$.ajax({
        type: "POST",
        url: base_url+'docadvsave/'+token_hash,
        data: dataa,
        cache: false,
		beforeSend: function(){
        ...
        success: function (resp) {
        	...
        	if(resp.data=='success') {
        		setTimeout(function(){
                    window.location.href = base_url+'doc_checklist';
                 }, 250);
